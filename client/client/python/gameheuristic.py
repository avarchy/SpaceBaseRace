            ...
            for i in range(0, N * N):
                ...
                for rotations in range(0, 4):
                    ...
                    if self.can_place(new_block, Point(x, y)):
                        tempheur = heuristic(new_block)
                        if(tempheur > bestheuristic):
                            bestmove = (index, rotations, x, y)
                            bestheuristic = tempheur
        return bestmove

    def heuristic(self, block):
        numblockscanplace = len(blocks[blockind])
        distweight = 0
        for dogecoin in bonus_squares:
            for offset in blocks:
                p = point + offset
                x = p.x
                y = p.y
                distweight = min(distweight, p.distance(dogecoin))
        if(distweight==1):
            distweight = 99999
        return 10*numblockscanplace+numblockscanplace*1000/distweight
